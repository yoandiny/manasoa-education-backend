...
import pool from './bdd.js'; // Assurez-vous que votre module `bdd.js` est également compatible avec ESM.
import { error } from 'console';

export const makeReportClassCard = async (
  student_id, 
  quarter_id, 
  reportModel, 
  gradeRow, 
  ...
  coeffSumCell, 
  coeffSumCell2, 
  definitiveNotesCell1,
  definitiveNotesCell2, 
  averageCell, 
  studentCounterCell, 
  maxAverageCell, 
  classAverageCell, 
  ...
  const reportPath = path.resolve('reportCards', `${student.rows[0].last_name}-ReportCard.xlsx`);
  await workbook.xlsx.writeFile(reportPath);
  
  return reportPath;  // Retourne le chemin du fichier généré
};

export const generateClassReport = async (student_id, quarter_id) => {
  if (student_id && quarter_id) {
    ...
    if (class_id.rows[0].class_id <= 3) {
      ...
      let reportModel = path.resolve('assets', 'Terminale_report.xlsx');
      ...
      if (class_id.rows[0].class_id <= 7) {
        let reportModel = path.resolve('assets', 'HighSchool_report.xlsx');
        ...
        if (class_id.rows[0].class_id == 8) {
          let reportModel = path.resolve('assets', 'Third_report.xlsx');
          ...
          if (class_id.rows[0].class_id <= 11) {
            let reportModel = path.resolve('assets', 'College_report.xlsx');
            ...
            if (class_id.rows[0].class_id == 12) {
              let reportModel = path.resolve('assets', 'CM2_report.xlsx');
              ...
              if (class_id.rows[0].class_id <= 14) {
                let reportModel = path.resolve('assets', 'Primaire_mid_report.xlsx');
                ...
                if (class_id.rows[0].class_id == 15) {
                  let reportModel = path.resolve('assets', 'CE1_report.xlsx');
                ...
                if (class_id.rows[0].class_id == 16) {
                  let reportModel = path.resolve('assets', 'CP_report.xlsx');
                  ...
                  if (class_id.rows[0].class_id == 17) {
                    let reportModel = path.resolve('assets', 'MGS_report.xlsx');
                    ...
                    if (class_id.rows[0].class_id <= 19) {
                      let reportModel = path.resolve('assets', 'Presco_report.xlsx');
                      ...
                      if (class_id.rows[0].class_id == 20) {
                        let reportModel = path.resolve('assets', 'TPS_report.xlsx');
